...
class A3C(Base_Agent):
    ...
    def run_n_episodes(self):
        ...
        episodes_per_process = (
            int(self.config.num_episodes_to_run / self.worker_processes) + 1
        )
        processes = []
...
class Actor_Critic_Worker(torch.multiprocessing.Process):
        ...
        for ep_ix in range(self.episodes_to_run):
            ...
            while not done:
                action, action_log_prob, critic_outputs = (
                    self.pick_action_and_get_critic_values(
                        self.local_model, state, epsilon_exploration
                    )
                )
                next_state, reward, done, truncate, _ = self.environment.step(action)
    ...
    def pick_action_and_get_critic_values(
        self, policy, state, epsilon_exploration=None
    ):
        """Picks an action using the policy"""
        state = torch.from_numpy(state).float().unsqueeze(0)
        model_output = policy.forward(state)
        actor_output = model_output[
